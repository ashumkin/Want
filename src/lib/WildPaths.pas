...
function SplitListToPaths(const sList: string): TPaths;
...
function PathArraySortCompare(Item1, Item2: Pointer): Integer;
begin
  // сортируем в обратном пор€дке, чтобы, например,
  // C:\Components\Want шло раньше, чем C:\Components,
  // и соответственно раньше удал€лось, иначе корневые папки не удал€ютс€ совсем
  // возможно, лучше сортировать не по алфавиту, а по "глубине" пути
...
var
  f: Integer;
begin
  SortDynArray(Files, SizeOf(Files[0]), @PathArraySortCompare);
...
begin
  ...
  try
    ExtractStrings([',', ';'], [' '], PAnsiChar(sList), TSL);
    Result := StringsToPaths(TSL);
  finally
    FreeAndNil(TSL);
  end;
end;

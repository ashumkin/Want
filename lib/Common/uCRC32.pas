...
type
  ECRC32 = class(Exception);

function FileCRC32(const FileName: string): Cardinal;
function StreamCRC32(Stream : TStream): Cardinal;
...
begin
  ...
  crc32 := $FFFFFFFF;
  try
    InFileStream := TFileStream.Create(FileName, fmShareDenyNone);
    try
      Res := InFileStream.Read(Buf, BufLen);
    ...
    on E: Exception do
      raise ECRC32.CreateFmt('Error while processing file "%s": %s', [FileName, E.Message]);
  end;
  Result := not crc32;
end;
